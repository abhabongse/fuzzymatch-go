...
/*
MakeOptimalAlignmentDistFunc is a higher-order function which receives the substitution
and the transposition penalty functions and returns the modified version of the otherwise
generic OptimalAlignmentDist function with those specified edit distance metrics.
The resulting function will receive two input strings and output the distance between them.
*/
func MakeOptimalAlignmentDistFunc(substPenalty, transPenalty RunePenaltyFunc) StringDistFunc {
	return func(fst, snd string) float64 {
		return OptimalAlignmentDist(fst, snd, substPenalty, transPenalty)
	}
}

/*
SimpleAlignmentDist is a simplified version of the OptimalAlignmentDist function.
...
*/
var SimpleAlignmentDist = MakeOptimalAlignmentDistFunc(UnitPenalty, UnitPenalty)

/*
OptimalAlignmentDist computes the "optimal alignment distance" between two given
...
expect from the substDistFunc, see the distance metric function UnitPenalty.

For example, under the UnitPenalty character substitution metric, the Levenshtein distance
...
For example, under the UnitPenalty character substitution and transposition metrics, the
...
func OptimalAlignmentDist(
	fst, snd string,
	substitutionDistFunc, transpositionDistFunc RunePenaltyFunc,
) float64 {
...
the function OptimalAlignmentDist, respectively. Returned indices
are in modulus 3.
*/
...
the function OptimalAlignmentDist, respectively.
*/
